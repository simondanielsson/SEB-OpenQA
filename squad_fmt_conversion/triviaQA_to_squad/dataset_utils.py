...
def answer_index_in_document(answer, document):
    answer_list = answer['Aliases']
    answers = []
    indices = []
    for answer_string_in_doc in answer_list:
        index = document.find(answer_string_in_doc) #document.lower().find(answer_string_in_doc)
        if index != -1:
            answers.append(answer_string_in_doc)
            indices.append(index)
    if answers and indices:
